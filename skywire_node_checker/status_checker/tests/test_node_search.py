...
from skywire_node_checker.status_checker.features.node_search import node_search
...
class AutoUpdateTestCase(TestCase):
	...
	def setUp(self):
		...
		self.response_public_key_list = ['02a30a4684e915dc5b8225ad105ca2c9c2844a97982d3d0042e856670fb1285512', '020907316c650dcc89629f8a1960e2309228d31718da41274f67b4c947d1006f68', '03b30c57928197913dae276165e03cda06e7c86ce9a50917f91bd784101256c81d', '020c35929e7042d03db01777b32f9a200ed4ac716d1bc99d6704c8686495a29e14', '02f4e21fda5d6025d78cd08529a40d99a294991106718d7b97bc51503e850e7d4e' ]
	...
	def test_search_all_online(self):
		request = None
		expected_node_list_size = 5
		with mock.patch("skywire_node_checker.status_checker.features.node_search.is_form_filled", return_value=[self.response_public_key_list, True]):
			node_list, show_form = node_search(request)

		self.assertEqual(expected_node_list_size, len(node_list))
		self.assertTrue(show_form)

		for node in node_list:
			self.assertTrue(node.online)

	def test_search_one_offline(self):
		request = None
		...
		for node in node_list:
			if node.key == self.key_1:
				self.assertFalse(node.online)
			else:
				self.assertTrue(node.online)

	def test_search_offline_to_online(self):
		request = None
		with mock.patch("skywire_node_checker.status_checker.utils.do_request", return_value=self.response_key_1_offline ):
			autoupdate()
	...
	def test_search_total_uptime(self):
		request = None
		...
		for node in node_list:
			if node.key == self.key_1:
				self.assertEqual(expected_total_uptime, node.total_uptime)
